...
import paramiko, psutil, requests, platform, time

class Server(models.Model):
    _name = 'domain.server'  # name_of_module.name_of_class
        ...
        for record in data:
            try:
                ...
                if ram_percent >= 80 and disk_percent >= 80:
                    text = f"{record.id}-{record.name_server}-D(DISK {disk_percent:.2f}%|{used_disk:.0f}GB|BAD)-M(RAM {ram_percent:.2f}%|{used_ram:.0f}GB|BAD) @krisna & @ariefalrasyid"
                    self.send_telegram_message(text)  # Pass the 'message' argument
                
                elif ram_percent >= 80 and disk_percent <= 80:
                    text = f"{record.id}-{record.name_server}-D(DISK {disk_percent:.2f}%|{used_disk:.0f}GB|GOOD)-M(RAM {ram_percent:.2f}%|{used_ram:.0f}GB|BAD) @krisna & @ariefalrasyid"
                    self.send_telegram_message(text)  # Pass the 'message' argument

                elif ram_percent <= 80 and disk_percent >= 80:
                    text = f"{record.id}-{record.name_server}-D(DISK {disk_percent:.2f}%|{used_disk:.0f}GB|BAD)-M(RAM {ram_percent:.2f}%|{used_ram:.0f}GB|GOOD) @krisna & @ariefalrasyid"
                    self.send_telegram_message(text)  # Pass the 'message' argument

                else:
                    text = f"{record.id}-{record.name_server}-D(DISK {disk_percent:.2f}%|{used_disk:.0f}GB|GOOD)-M(RAM {ram_percent:.2f}%|{used_ram:.0f}GB|GOOD)"
                    self.send_telegram_message(text)  # Pass the 'message' argument 
                print("TESTING masuk")
                time.sleep(60)
