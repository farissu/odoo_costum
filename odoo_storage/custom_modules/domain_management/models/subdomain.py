        ...
        for record in website_url:
            ...
            try:
                if r.status_code != 200:
                    ...
                    message = "Domain " + str(record.name) + " " "DOWN" "\npada" " " + str(format_date) + "\nStatus Code" " " + str(r.status_code) + "&disable_web_page_preview=true @krisna"
                    ...
                    if domain:
                        ...
                        for datas in domain:
                            ...
                            if not datas.start :
                                ...
                                if datas.start :
                                    message = "Domain " + str(record.name) + " " "UP" "\npada" " " + str(format_date) + "\nStatus Code" " " + str(r.status_code) + "\nWaktu down" " " + str(datas.time_down) + "&disable_web_page_preview=true @krisna "
                                    url = "https://api.telegram.org/bot" + str(bot_info.botID) +"/sendMessage?text=" + str(message) + "&chat_id=" + str(bot_info.chatID)
                                    requests.get(url)
                    
            except requests.exceptions.SSLError :
                message = "Domain " + str(record.name) + " " "SSL ERROR" "\npada" " " + "&disable_web_page_preview=true @krisna "
                url = "https://api.telegram.org/bot" + str(bot_info.botID) +"/sendMessage?text=" + str(message) + "&chat_id=" + str(bot_info.chatID)
                requests.get(url)
